...
func updateNotification(linkURL *url.URL, org, repo, branch, commandURL string, latestNotification *comment, approversHandler approvers.Approvers) *string {
	message := getMessageWrapper(linkURL, org, repo, branch, commandURL,approversHandler)
...
func getMessageWrapper(linkURL *url.URL, org, repo, branch, commandURL string, approversHandler approvers.Approvers) *string{
	message := approvers.GetMessage(approversHandler, linkURL, org, repo, branch)
	if message == nil {
		return message
	}
	replaceStr := "https://go.k8s.io/bot-commands"
	repMsg := strings.Replace(*message, replaceStr, commandURL, 1)
	return &repMsg
}
